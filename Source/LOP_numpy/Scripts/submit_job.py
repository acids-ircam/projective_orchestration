...
import LOP.Scripts.train_wrapper as train_wrapper
import re
...
def submit_job(config_folder_fold, parameters, model_params, dimensions, K_fold, 
	...
	save_bool, generate_bool, local, logger):
	
	context_folder = config_folder_fold + '/context'
	os.mkdir(context_folder)

	# Build track path generation
	track_paths_generation = [config.database_root() + '/' + e for e in test_names]
