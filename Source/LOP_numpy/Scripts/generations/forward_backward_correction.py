...
from generate_backward import generate_backward
...
def generate_midi(config_folder_fd, config_folder_bd, config_folder_corr, score_source, save_folder, initialization_type, number_of_version, duration_gen, num_pass_correct, logger_generate):
    ...
    #
    parameters_bd = pkl.load(open(config_folder_bd + '/script_parameters.pkl', 'rb'))
    model_parameters_bd = pkl.load(open(config_folder_bd + '/model_params.pkl', 'rb'))
    #
    parameters_corr = pkl.load(open(config_folder_corr + '/script_parameters.pkl', 'rb'))
    model_parameters_corr = pkl.load(open(config_folder_corr + '/model_params.pkl', 'rb'))

    assert (model_parameters_fd["temporal_order"]==model_parameters_bd["temporal_order"]) and (model_parameters_fd["temporal_order"]==model_parameters_corr["temporal_order"]), "The two model have different seed_size"
    assert (parameters["quantization"]==parameters_bd["quantization"]) and (parameters["quantization"]==parameters_corr["quantization"]), "The two model have different quantization"
    assert (parameters["temporal_granularity"]==parameters_bd["temporal_granularity"]) and (parameters["temporal_granularity"]==parameters_corr["temporal_granularity"]) , "The two model have different temporal_granularity"
    assert (parameters["instru_mapping"]==parameters_bd["instru_mapping"]) and (parameters["instru_mapping"]==parameters_corr["instru_mapping"]) , "The two model have different instru_mapping"
    assert (parameters["normalizer"]==parameters_bd["normalizer"]) and (parameters["normalizer"]==parameters_corr["normalizer"]) , "The two model have different normalizer"
    ...
    if re.search(r'mid$', score_source):
        pr_piano, event_piano, duration_piano, name_piano, pr_orch, instru_orch, duration = generation_utils.load_solo(score_source, parameters["quantization"], parameters["binarize_piano"], parameters["temporal_granularity"])
    else:
        if initialization_type=="seed":
            pr_piano, event_piano, duration_piano, name_piano, pr_orch, instru_orch, duration = generation_utils.load_from_pair(score_source, parameters["quantization"], parameters["binarize_piano"], parameters["binarize_orch"], parameters["temporal_granularity"], align_bool=True)
        else:
            pr_piano, event_piano, duration_piano, name_piano, pr_orch, instru_orch, duration = generation_utils.load_from_pair(score_source, parameters["quantization"], parameters["binarize_piano"], parameters["binarize_orch"], parameters["temporal_granularity"], align_bool=False)
    ...
    pr_orchestra_truth = build_data_aux.cast_small_pr_into_big_pr(pr_orch, instru_orch, 0, duration_gen, parameters["instru_mapping"], pr_orchestra_truth)
    if initialization_type == "seed":
        pr_orchestra_seed = generation_utils.init_with_seed(pr_orch, number_of_version, seed_size, N_orchestra, instru_orch, parameters["instru_mapping"])
    ...
    # Trainer
    trainer_bd = import_trainer(which_trainer_bd, model_parameters_bd, parameters)
    #
    ...
    # Forward
    pr_orchestra_gen = generate(trainer_fd, pr_piano_gen_norm, silence_piano, duration_piano, config_folder_fd, model_path, pr_orchestra_seed, batch_size=number_of_version)
    prefix_name = 'fd_'
    ...
    pr_orchestra_gen = generate_backward(trainer_bd, pr_piano_gen_norm, silence_piano, duration_piano, config_folder_bd, model_path, pr_orchestra_seed, batch_size=number_of_version)
    prefix_name = 'bd_'
    generation_utils.reconstruct_generation(pr_orchestra_gen, event_piano, generated_folder, prefix_name, parameters, seed_size)
    # Correction
...
if __name__ == '__main__':
    ...
    # config_folder_fd = '/Users/leo/Recherche/lop/LOP/Results/Data_bp_bo_noEmb_tempGran32/LSTM_plugged_base/TEST/0'
    # config_folder_bd = '/Users/leo/Recherche/lop/LOP/Results/Data_bp_bo_noEmb_tempGran32/LSTM_plugged_base_BW/TEST/0'
    # config_folder_corr = '/Users/leo/Recherche/lop/LOP/Results/Data_bp_bo_noEmb_tempGran32/LSTM_correction_vector_wise/TEST/0'
    
    config_folder_fds = ['/fast-1/leo/automatic_arrangement/Saved_models/LSTM_fd/' + str(e) for e in range (10)]
    config_folder_bds = ['/fast-1/leo/automatic_arrangement/Saved_models/LSTM_bd/' + str(e) for e in range (10)]
    config_folder_corrs = ['/fast-1/leo/automatic_arrangement/Saved_models/LSTM_corr/' + str(e) for e in range (10)]
    
    for config_folder_fd, config_folder_bd, config_folder_corr in zip(config_folder_fds, config_folder_bds, config_folder_corrs):
        score_sources = []
        ...
        for initialization_type in ["zeros", "random", "constant"]:
            for score_source in score_sources:
                generate_midi(config_folder_fd, config_folder_bd, config_folder_corr, score_source, save_folder, initialization_type=initialization_type, number_of_version=3, duration_gen=100, num_pass_correct=5, logger_generate=logger)