...
class Standard_trainer(object):
	...
	def build_variables_nodes(self, model, parameters):
		# Build nodes
		# Inputs
		self.piano_t_ph = tf.placeholder(self.tf_type, shape=(None, model.piano_dim), name="piano_t")
		self.piano_past_ph = tf.placeholder(self.tf_type, shape=(None, self.temporal_order-1, model.piano_dim), name="piano_past")
		self.piano_future_ph = tf.placeholder(self.tf_type, shape=(None, self.temporal_order-1, model.piano_dim), name="piano_future")
		#
		self.orch_t_ph = tf.placeholder(self.tf_type, shape=(None, model.orch_dim), name="orch_t")
		self.orch_past_ph = tf.placeholder(self.tf_type, shape=(None, self.temporal_order-1, model.orch_dim), name="orch_past")
		self.orch_future_ph = tf.placeholder(self.tf_type, shape=(None, self.temporal_order-1, model.orch_dim), name="orch_past")
		# Orchestral mask
		self.mask_orch_ph = tf.placeholder(self.tf_type, shape=(None, model.orch_dim), name="mask_orch")
		return
	...
	def build_distance(self, model, parameters):
		...
		# distance = accuracy_low_TN_tf(self.orch_t_ph, self.preds, model.WW)
		# distance = bin_Xen_weighted_1_tf(self.orch_t_ph, self.preds, weight_neg=model.WW)
		# distance = bin_Xen_weighted_0_tf(self.orch_t_ph, self.preds, parameters["activation_ratio"], weight_neg=model.WW)
		# distance = bin_Xen_weighted_Positive(self.orch_t_ph, self.preds, parameters["activation_ratio"])
		# distance = keras.losses.mean_squared_error(self.orch_t_ph, self.preds)
		return distance
	...
	def build_loss_nodes(self, model, parameters):
		with tf.name_scope('loss'):
			with tf.name_scope('distance'):
				...
				self.accuracy = accuracy_tf(self.orch_t_ph, self.preds)
			...
			# Mask orchestration (if instru absent)
			# That was a test, mainly failed
