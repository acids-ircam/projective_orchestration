...
class NADE_informed_trainer(Standard_trainer):
	
	def __init__(self, **kwargs):
		super().__init__(**kwargs)		
		# Number of ordering used when bagging NADEs
		self.num_ordering = kwargs["num_ordering"]
		self.orch_to_orch = kwargs["orch_to_orch"]
		self.piano_to_orch = kwargs["piano_to_orch"]
		return
	...
	def orderless_NADE_generation(self, sess, feed_dict, orch_t):
		...
		correl_link = np.matmul(piano_t, self.piano_to_orch)
		indices_to_sample = np.where(correl_link > 0)[1]
		indices_NOT_sample = np.where(correl_link == 0)[1]
		...
		for ordering_ind in range(self.num_ordering):
			...
			orderings.append(indices_NOT_sample.tolist() + indices_to_sample.tolist())
	...
	def valid_step(self, sess, batch_index, piano, orch, duration_piano, mask_orch):
		...
		for ii, batch_ind in enumerate(batch_index):
			...
			this_loss_batch, this_preds_batch = self.generate_mean_ordering(sess, feed_dict, this_orch_t)
