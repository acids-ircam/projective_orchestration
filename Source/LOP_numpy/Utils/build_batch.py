...
def build_batch_seq(batch_index_list, piano, orch, duration_piano, mask_orch, batch_size, temporal_order):
    batch_index = np.asarray(batch_index_list)
    # Add duration ?
    if duration_piano is not None:
        piano = np.concatenate((piano, duration_piano.reshape([-1,1])), axis=1)
    # Build batch
    piano = build_sequence(piano, batch_index+1, batch_size, temporal_order)
    if len(orch.shape) == 2:
        orch = build_sequence(orch, batch_index+1, batch_size, temporal_order)
        orch_tm1 = build_sequence(orch, batch_index, batch_size, temporal_order)
    elif len(orch.shape) == 3:
        orch = build_sequence_from_3D_matrix(orch, batch_index+1, temporal_order)
        orch_tm1 = build_sequence_from_3D_matrix(orch, batch_index, temporal_order)        
    return piano, orch_tm1, orch
