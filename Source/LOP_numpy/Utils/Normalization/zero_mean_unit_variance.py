...
class zero_mean_unit_variance(object):
    """Zero mean unit variance object
    """
    ...
    def get_mean(self, train_folds, parameters):
        ...
        for train_fold in train_folds:
            piano, _, _, _ = load_matrices(train_fold["chunks_folders"], parameters, np.float32)
            if self.norm_dim is None:
                self.norm_dim = piano.shape[1]
            self.mean += np.sum(piano, axis=0)
            length += piano.shape[0]
        self.mean /= length
        return
    ...
    def get_var(self, train_folds, parameters):
        ...
        for train_fold in train_folds:
            piano, _, _, _ = load_matrices(train_fold["chunks_folders"], parameters, np.float32)
            self.var += np.sum(np.square(piano - self.mean), axis=0)
            length += piano.shape[0]
        self.var /= length
        return
