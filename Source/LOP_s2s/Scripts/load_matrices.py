...
def load_matrices(chunk_path_list, parameters):
    ...
    orch_transformed = np.zeros((T_max, parameters["N_orchestra"]), dtype=np.float32)
    if parameters["embedded_piano"]:
        piano_input = np.zeros((T_max, parameters["N_piano_embedded"]), dtype=np.float32)
    else:
        piano_input = np.zeros((T_max, parameters["N_piano"]), dtype=np.float32)

    if parameters["duration_piano"]:
        duration_piano = np.zeros((T_max,), dtype=np.float32)

    if parameters["mask_orch"]:
        mask_orch = np.zeros((T_max, parameters["N_orchestra"]), dtype=np.float32)
