...
class NADE_comb_trainer(Standard_trainer):
	
	def __init__(self, **kwargs):
		super().__init__(**kwargs)		
		# Number of ordering used when bagging NADEs
		self.num_ordering = kwargs["num_ordering"]
		self.pitch_orch = kwargs["pitch_orch"]
		self.pitch_piano = kwargs["pitch_piano"]
		return
	...
	def training_step(self, sess, batch_index, piano, orch, duration_piano, mask_orch, summarize_dict):
		...
		for batch_ind in range(batch_size):
			...
			# We don't set min value to zero, it's never gonna be the case
			d = random.randint(10, orch_dim)
			# Indices
	...
	def generate_mean_ordering(self, sess, feed_dict, orch_t, instrumentation):
		orch_dim = orch_t.shape

		orch_pred, loss_batch = self.orderless_NADE_generation(sess, feed_dict, orch_t, instrumentation)
		
		##############################
		##############################
		# MEAN
		preds_mean_over_ordering = self.mean_parallel_prediction(orch_pred)
		preds_sample_from_ordering = np.random.binomial(1, preds_mean_over_ordering)
		##############################
		##############################
		# TAKE FIRST ONE
		# preds_mean_over_ordering = orch_pred[0]
		# preds_sample_from_ordering = np.random.binomial(1, preds_mean_over_ordering)
		##############################
		##############################

		loss_batch_mean = self.mean_parallel_prediction(loss_batch)
	...
	def mean_parallel_prediction(self, matrix):
		# Mean over the different generations (Comb filter output)
		mean_over_ordering = np.mean(matrix, axis=0)
		return mean_over_ordering

	def valid_step(self, sess, batch_index, piano, orch, duration_piano, mask_orch, instrumentation=None):
		...
		if len(orch.shape)==2:
			...
			orch_dim = orch.shape[2]
		else:
			raise Exception("bad shape for orchestra")

		preds_batch = np.zeros((len(batch_index), orch_dim))
		orch_t = np.zeros((len(batch_index), orch_dim))
		loss_batch = []

		# for batch_ind in batch_index:
		for ii, batch_ind in enumerate(batch_index):
			if generating_flag:
				feed_dict, this_orch_t = super().build_feed_dict([batch_ind], piano, orch[ii], duration_piano, mask_orch)
			else:
				feed_dict, this_orch_t = super().build_feed_dict([batch_ind], piano, orch, duration_piano, mask_orch)
			this_loss_batch, this_preds_batch = self.generate_mean_ordering(sess, feed_dict, this_orch_t, instrumentation)
			preds_batch[ii] = this_preds_batch
			orch_t[ii] = this_orch_t
			loss_batch.append(this_loss_batch)
	...
	def generation_step(self, sess, batch_index, piano, orch_gen, duration_gen, mask_orch, instrumentation=None):
		# Exactly the same as the valid_step
		loss_batch, preds_batch, orch_t = self.valid_step(sess, batch_index, piano, orch_gen, duration_gen, mask_orch, instrumentation)
		return preds_batch