...
class correct_trainer_balanced(Standard_trainer):
	...
	def build_variables_nodes(self, model, parameters):
		self.piano_t_ph = tf.placeholder(tf.float32, shape=(1, model.piano_dim), name="piano_t")
		self.piano_past_ph = tf.placeholder(tf.float32, shape=(1, self.temporal_order-1, model.piano_dim), name="piano_past")
		self.piano_future_ph = tf.placeholder(tf.float32, shape=(1, self.temporal_order-1, model.piano_dim), name="piano_future")
		#
		self.orch_t_ph = tf.placeholder(tf.float32, shape=(1, model.orch_dim), name="orch_t")
		self.orch_past_ph = tf.placeholder(tf.float32, shape=(1, self.temporal_order-1, model.orch_dim), name="orch_past")
		self.orch_future_ph = tf.placeholder(tf.float32, shape=(1, self.temporal_order-1, model.orch_dim), name="orch_past")
		# Orchestral mask
		self.mask_orch_ph = tf.placeholder(tf.float32, shape=(1, model.orch_dim), name="mask_orch")
	...
	def build_loss_nodes(self, model, parameters):
		with tf.name_scope('distance'):
			orch_t_masked = tf.boolean_mask(self.orch_t_ph[0], 1-self.pitch_mask)
			preds_masked = tf.boolean_mask(self.preds[0], 1-self.pitch_mask)
			# These are [batch, 1] size vectors
			distance = keras.losses.binary_crossentropy(orch_t_masked, preds_masked)
	...
	def training_step(self, sess, batch_index, piano, orch, duration_piano, mask_orch, summarize_dict):
		...
		for batch_ind in range(len(batch_index)):
			feed_dict = {self.piano_t_ph: piano_t[[batch_ind]],
			self.piano_past_ph: piano_past[[batch_ind]],
			self.piano_future_ph: piano_future[[batch_ind]],
			self.orch_past_ph: orch_past[[batch_ind]],
			self.orch_future_ph: orch_future[[batch_ind]],
			self.orch_t_ph: orch_t[[batch_ind]],
			self.mask_orch_ph: mask_orch_t[[batch_ind]]}
		...
		final_loss = np.mean(loss_batches)

		return final_loss, preds_batch, debug_outputs, summary
