...
def generate(trainer, piano, silence_ind, duration_gen, path_to_config, model_name='model', orch_init=None, batch_size=5, mask_instrumentation=None):
    ...
    orch_gen = np.zeros((batch_size, total_length+temporal_order, orch_dim))   # + temporal order is padding for trainer.generation_step which accepts only time indices not close to limits of time series
    orch_gen[:, :init_length, :] = orch_init
    # Pad piano
    N_piano = piano.shape[1]
    piano_padded = np.concatenate([piano, np.zeros([temporal_order, N_piano])], axis=0)
    if duration_gen is not None:
        duration_gen_padded = np.concatenate([duration_gen, np.zeros([temporal_order])])
        duration_gen_padded = np.reshape(duration_gen_padded, [-1, 1])
    else:
        duration_gen_padded = None
    ...
    with tf.Session(config=configSession) as sess:
        ...
        for t in range(init_length, total_length):
            ...
            if t not in silence_ind:
                ...
                if mask_instrumentation is None:
                    prediction = trainer.generation_step(sess, batch_index, piano_padded, orch_gen, duration_gen_padded, None)
                else:
                    prediction = trainer.generation_step(sess, batch_index, piano_padded, orch_gen, duration_gen_padded, None, mask_instrumentation)
