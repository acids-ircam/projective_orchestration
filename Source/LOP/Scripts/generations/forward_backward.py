...
def generate_midi(config_folder_fd, config_folder_bd, score_source, save_folder, initialization_type, number_of_version, duration_gen, number_fdbd_pass, logger_generate):
    ...
    assert (model_parameters_fd["temporal_order"]==model_parameters_bd["temporal_order"]) , "The two model have different seed_size"
    assert (parameters["quantization"]==parameters_bd["quantization"]) , "The two model have different quantization"
    assert (parameters["temporal_granularity"]==parameters_bd["temporal_granularity"]) , "The two model have different temporal_granularity"
    assert (parameters["instru_mapping"]==parameters_bd["instru_mapping"]) , "The two model have different instru_mapping"
    assert (parameters["normalizer"]==parameters_bd["normalizer"]) , "The two model have different normalizer"
...
if __name__ == '__main__':
    ...
    for config_folder_fd, config_folder_bd in zip(config_folder_fds, config_folder_bds):
        score_sources = []
        ...
        for initialization_type in ["zeros", "random", "constant"]:
            for score_source in score_sources:
                generate_midi(config_folder_fd, config_folder_bd, score_source, save_folder, initialization_type=initialization_type, number_of_version=3, duration_gen=100, number_fdbd_pass=5, logger_generate=logger)