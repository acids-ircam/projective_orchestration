...
class LSTM_conv_base(Model_lop):
    ...
    def get_hp_space():
        ...
        space = {
            ...
            'num_filter': hopt_wrapper.qloguniform_int('num_filter', log(10), log(100), 5),
            'filter_size': hopt_wrapper.quniform_int('filter_size', 6, 24, 1),
        }
    ...
    def predict(self, inputs_ph):
        ...
        with tf.name_scope("piano_embedding"):
            conv_layer = Conv1D(self.num_filter, self.filter_size, activation='relu')
            piano_t_reshape = tf.reshape(piano_t, [-1, self.piano_dim, 1])
            piano_embedding = conv_layer(piano_t_reshape)
            piano_embedding = tf.reshape(piano_embedding, [-1, (self.piano_dim-self.filter_size+1) * self.num_filter])
