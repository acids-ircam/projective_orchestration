...
class cRBM(Model_lop):
    ...
    def init_weights(self):
        self.weights = {}
        self.weights['W'] = tf.get_variable("W", (self.n_visible, self.n_hidden), initializer=tf.random_normal_initializer(stddev=0.01), dtype=tf.float64)
        self.weights['A'] = tf.get_variable("A", (self.n_condition, self.n_visible), initializer=tf.random_normal_initializer(stddev=0.01), dtype=tf.float64)
        self.weights['B'] = tf.get_variable("B", (self.n_condition, self.n_hidden), initializer=tf.random_normal_initializer(stddev=0.01), dtype=tf.float64)
        self.weights['a_stat'] = tf.get_variable("a_stat", (self.n_visible), initializer=tf.zeros_initializer(), dtype=tf.float64)
        self.weights['b_stat'] = tf.get_variable("b_stat", (self.n_hidden), initializer=tf.zeros_initializer(), dtype=tf.float64)

        for key, value in self.weights.items():
            variable_summary(value)
