...
def bin_Xen_weighted_0_tf(true_frame, pred_frame, activation_ratio, weight_neg=1):
    """Binary cross-entropy. Should be exactly the same as keras.losses.binary_crossentropy
    
    """
    # Avoid zeros weighting 
    zeros_ind = activation_ratio==0
    espsilon_activation = activation_ratio[1-zeros_ind].min()
    activation_ratio[zeros_ind] = espsilon_activation

    axis = len(true_frame.shape)-1
    epsilon = 1e-20
    cross_entr_dot = (true_frame * tf.log(pred_frame+epsilon)) + weight_neg * activation_ratio * (1-true_frame) * tf.log((1-pred_frame+epsilon))

    # cross_entr_dot = (true_frame * tf.log(pred_frame+epsilon)) * weight_neg * (1/activation_ratio) + (1-true_frame) * tf.log((1-pred_frame+epsilon))
    
    # cross_entr_dot = (true_frame * tf.log(pred_frame+epsilon)) * (- weight_neg * math.log(activation_ratio)) + (1-true_frame) * tf.log((1-pred_frame+epsilon))
...
def bin_Xen_weighted_Positive(true_frame, pred_frame, activation_ratio):
    """Binary cross-entropy. Should be exactly the same as keras.losses.binary_crossentropy
    
    """
    # Avoid zeros weighting 
    zeros_ind = activation_ratio<1e-10
    espsilon_activation = activation_ratio[1-zeros_ind].min()
    activation_ratio[zeros_ind] = espsilon_activation
    ...
    cross_entr_dot = (- np.log(activation_ratio)) * (true_frame * tf.log(pred_frame+epsilon)) + (1-true_frame) * tf.log((1-pred_frame+epsilon)) 
