...
def build_one_fold(k, k_folds, t_dict, tv_dict, tvt_dict, t_names_shuffle, tv_names_shuffle, tvt_names_shuffle, temporal_order, train_batch_size, long_range_pred, num_max_contiguous_blocks, embedded_piano_bool, mask_orch_bool, random_inst):
    ...
    for counter, filename in enumerate(tvt_names_shuffle):
    ...
    for counter, filename in enumerate(tv_names_shuffle):
        counter_fold = counter + k
        if (counter_fold % k_folds) < k_folds-2:
            this_train_names.append(filename)
    ...
    for filename in t_names_shuffle:
        this_train_names.append(filename)
        split_matrices_train.extend(t_dict[filename])
    
    this_fold = {"train": from_block_list_to_folds(split_matrices_train, temporal_order, train_batch_size, long_range_pred, num_max_contiguous_blocks, embedded_piano_bool, mask_orch_bool, random_inst),
        "valid": from_block_list_to_folds(split_matrices_valid, temporal_order, train_batch_size, long_range_pred, num_max_contiguous_blocks, embedded_piano_bool, mask_orch_bool, random_inst),
        "test": from_block_list_to_folds(split_matrices_test, temporal_order, train_batch_size, long_range_pred, num_max_contiguous_blocks, embedded_piano_bool, mask_orch_bool, random_inst)}
    return this_fold, this_train_names, this_valid_names, this_test_names


def from_block_list_to_folds(list_blocks, temporal_order, train_batch_size, long_range_pred, num_max_contiguous_blocks, embedded_piano_bool, mask_orch_bool, random_inst):
    ...
    for block_folder in list_blocks:
        ...
        pr_piano, _, pr_orch, _, _ = load_matrices.load_matrix_NO_PROCESSING(block_folder, embedded_piano_bool, mask_orch_bool)
